...
function EncounterCard({ encounter, onCardClick, darkMode }) {
  ...
  const cardClass = darkMode
    ? "bg-gray-800 border-green-700 hover:scale-105 transition-transform duration-200 cursor-pointer shadow-md"
    : "bg-green-50 border border-green-200 hover:scale-105 transition-transform duration-200 cursor-pointer shadow-md";

  const titleClass = darkMode ? "text-green-300" : "text-green-700";
  const descClass = darkMode ? "text-gray-400" : "text-gray-500";
  const labelClass = darkMode ? "text-green-500 font-semibold" : "text-green-600 font-semibold";
...
function AddEncounterCard({ onCardClick, darkMode }) {
  const cardClass = darkMode
    ? "flex justify-center items-center border-2 border-dashed border-green-600 rounded-lg p-4 shadow-md bg-gray-800 cursor-pointer hover:scale-105 transition-transform duration-200 min-h-[150px]"
  ...
  const plusClass = darkMode ? "text-green-500 text-6xl font-light" : "text-green-500 text-6xl font-light";
...
export default function Encounters() {
  ...
  if (loading) return <p className={`p-6 ${darkMode ? "text-gray-200" : "text-gray-700"}`}>Loading encounters...</p>;
  ...
  return (
    <div className={`p-6 h-screen overflow-y-auto ${darkMode ? "bg-gray-900" : "bg-white"}`}>
      <h1 className={`${darkMode ? "text-green-300" : "text-green-700"} text-2xl font-semibold mb-2`}>
        My Encounters
      </h1>
